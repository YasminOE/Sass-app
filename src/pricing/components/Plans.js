...
export default function Plans({ plans }) {
  // handle plan interval button state
  const [selected, setSelected] = useState('month');
  const plan = plans.find(plan => plan.interval === selected);

  // handle buy now button state 
  ...
  // Network request from backend to frontend
  async function onCheckout(){
    setRedirecting(true);
 
    // return the created session id from the checkout endpoint
